        ...
        <div class="tab-content" id="v-pills-tabContent">
                            ...
                            <table class="table table-striped table-hover align-middle">
                                ...
                                <tbody>
                                    @forelse($data_tutor as $key => $t)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td class="fw-bold text-success">{{ $t->name }}</td>
                                            <td>{{ $t->email }}</td>
                                ...
                                <tbody>
                                    @forelse($data_pendaftar as $key => $p)
                                        <tr>
                                            ...
                                            <td>
                                                @if (($p->status ?? 'Pending') == 'Diterima' || ($p->status ?? 'Pending') == 'Disetujui')
                                                    <span class="badge bg-success">Disetujui</span>
                                                @elseif(($p->status ?? 'Pending') == 'Ditolak')
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    @if (($p->status ?? 'Pending') == 'Pending')
                                                        <form
                                                            action="{{ route('admin.pendaftaran.status', ['id' => $p->id, 'status' => 'Diterima']) }}"
                                                            method="POST" class="d-inline m-0">
                                                            @csrf
                                                            <button type="submit"
                                                        ...
                                                        <form
                                                            action="{{ route('admin.pendaftaran.status', ['id' => $p->id, 'status' => 'Ditolak']) }}"
                                                            method="POST" class="d-inline m-0">
                                                            @csrf
                                                            <button type="submit"
                            ...
                            <tbody>
                                ...
                                <tr class="border-bottom">
                                    ...
                                    <td class="py-3">
                                        @if (Auth::user()->keahlian)
                                            <span
                                                class="badge bg-info-subtle text-info px-2 py-1 fw-semibold">{{ Auth::user()->keahlian }}</span>
                                        @else
                                            <span
                                ...
                                <tr>
                                    ...
                                    <td class="text-secondary py-3">
                                        {{ Auth::user()->no_hp ?? 'Belum diisi oleh Admin' }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div class="tab-pane fade" id="panel-materi" role="tabpanel" aria-labelledby="tab-materi">
                ...
                <div class="modal fade" id="modalBuatKelasTutor" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <form action="{{ route('tutor.prosesSimpanKelas') }}" method="POST"
                            class="modal-content border-0 shadow-lg rounded-4">
                            @csrf
                            <div class="modal-header bg-dark text-white">
